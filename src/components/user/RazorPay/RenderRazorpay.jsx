...
const RenderRazorpay = ({ orderDetails, setDisplayRazorpay }) => {
  ...
  // Handles payment success/failure callback to backend
  const handlePayment = useCallback(async (status, paymentDetails) => {
    try {
  ...
  }, [orderDetails, setDisplayRazorpay, handlePayment, paymentSuccess]);

  // ✅ useEffect now includes displayRazorpay safely
  useEffect(() => {
    if (!paymentSuccess) {
  ...
  }, [paymentSuccess, displayRazorpay]);
